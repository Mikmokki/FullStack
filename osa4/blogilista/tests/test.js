...
describe('total likes', () => {
  ...
  test('when list has only one blog equals the likes of that', () => {
    expect(listHelper.totalLikes([one])).toBe(12)
  })
...
describe('favourite', () => {
  ...
  test('when list has only one blog equals itself', () => {
    expect(listHelper.favoriteBlog([one])).toEqual(one)
  })
  test('of a bigger list is calculated right', () => {
    expect(listHelper.favoriteBlog(blogs)).toEqual(one)
  })
})
...
describe('returns author with largest amount of likes', () => {
  ...
  test('when list has only one blog equals the author of that', () => {
    const result = listHelper.mostLikes([one])
    expect(result).toEqual({
      author: 'Edsger W. Dijkstra',
      likes: 12,
    })
